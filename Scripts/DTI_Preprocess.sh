...
pe_dir="AP"
readout_time="0.062"
while getopts 'hus:d:o:p:t:T:m:w:n' OPTION; do
  case "$OPTION" in
    [h,u])
      usage
      exit 1
...
if [ "$denoise" == "False" ]; then
    ...
    log_run /usr/local/lib/python3.12/dist-packages/diciphr/scripts/dti_preprocess.py -s $subject --no-denoise \
    -d $dwi -o $outdir -T $readout_time \
    --logfile $logfile --workdir $tmpdir \
    $topupflag $maskflag 
fi
