...
if [ ! -e "$tmpdir/t1_masked_imitT2.nii.gz" ]; then 
    perc90=$(/usr/local/lib/python3.12/dist-packages/afni/17.2.10/3dBrickStat -slow -percentile 90 1 90 -mask $tmpdir/t1_mask_1mm.nii.gz $tmpdir/t1_1mm.nii.gz | awk '{print $2}')
    log_run /usr/local/lib/python3.12/dist-packages/afni/17.2.10/3dcalc -overwrite -a $tmpdir/t1_1mm.nii.gz -prefix $tmpdir/t1_thresh.nii.gz -expr maxbelow\($perc90,a\) -float 
    log_run /usr/local/lib/python3.12/dist-packages/afni/17.2.10/3dUnifize -echo_edu -prefix $tmpdir/t1_thresh_uni.nii.gz -input $tmpdir/t1_thresh.nii.gz -overwrite 
    perc90=$(/usr/local/lib/python3.12/dist-packages/afni/17.2.10/3dBrickStat -slow -percentile 90 1 90 -mask $tmpdir/t1_mask_1mm.nii.gz $tmpdir/t1_thresh_uni.nii.gz | awk '{print $2}')
    # log_run /usr/local/lib/python3.12/dist-packages/afni/17.2.10/3dcalc -overwrite -a $tmpdir/t1_thresh_uni.nii.gz -b $tmpdir/t1_mask_1mm.nii.gz -prefix $tmpdir/t1_masked_imitT2.nii.gz -expr \(1.1\*${perc90}-a\)\*b+not\(b\)\*a -float 
    log_run /usr/local/lib/python3.12/dist-packages/afni/17.2.10/3dcalc -overwrite -a $tmpdir/t1_thresh_uni.nii.gz -b $tmpdir/t1_mask_1mm.nii.gz -prefix $tmpdir/t1_masked_imitT2.nii.gz -expr \(1.1\*${perc90}-a\)\*b -float 
fi 
